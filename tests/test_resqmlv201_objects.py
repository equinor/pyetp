...
def test_polyline_set_representation() -> None:
    ...
    # becomes an IntegerHdf5Array (not an IntegerConstantArray). Without this
    # the test fails ~25% of the time when random gives uniform lengths.
    ...
    # Same trick — guarantee a differing flag so closed becomes a
    # BooleanHdf5Array (not a BooleanConstantArray).
    if all(c == closed_in[0] for c in closed_in):
        closed_in[0] = not closed_in[0]

    crs = ro.obj_LocalDepth3dCrs(
        citation=ro.Citation(title="Polyline CRS", originator="pyetp-tester"),
        vertical_crs=ro.VerticalCrsEpsgCode(epsg_code=5715),
        projected_crs=ro.ProjectedCrsEpsgCode(epsg_code=23031),
    )

    epc = ro.obj_EpcExternalPartReference(
        citation=ro.Citation(title="Polyline epc", originator="pyetp-tester"),
    )

    pls, data_arrays = ro.obj_PolylineSetRepresentation.from_polylines(
        citation=ro.Citation(title="Polyline-set", originator="pyetp-tester"),
        crs=crs,
        epc_external_part_reference=epc,
        polylines=polylines,
        closed=closed_in,
    ...
    ret_pls, _ = compare_serialization_parsing_roundtrip(pls)
    assert isinstance(ret_pls, ro.obj_PolylineSetRepresentation)

    dor = ro.DataObjectReference.from_object(pls)
    assert (
        dor.content_type
        == "application/x-resqml+xml;version=2.0.1;type=obj_PolylineSetRepresentation"
    )
    assert dor.uuid == pls.uuid
    assert dor.title == pls.citation.title
    assert (
        dor.get_etp_data_object_uri("")
        == f"eml:///resqml20.obj_PolylineSetRepresentation({pls.uuid})"
    )

    assert ret_pls.line_role == ro.LineRole.INTERPRETATION_LINE
    assert len(ret_pls.line_patch) == 1
    patch = ret_pls.line_patch[0]
    assert patch.patch_index == 0
    assert isinstance(patch.node_count_per_polyline, ro.IntegerHdf5Array)
    assert isinstance(patch.closed_polylines, ro.BooleanHdf5Array)
    assert isinstance(patch.geometry.points, ro.Point3dHdf5Array)

    points, node_counts, closed_out = patch.decode(data_arrays)
